"""
Structural Knowledge Distillation (SKD) wrapper.
"""
...
from nnunetv2.utilities.kd_losses import SKD_Loss

from .base import DistillationMethod


class SKDDistillation(DistillationMethod, nn.Module):
    """
    Stage-wise structural knowledge distillation.

    Config parameters:
        student_channels (Sequence[int])
        teacher_channels (Sequence[int])  # optional but checked for length
    ...
    def __init__(self, **config):
        ...
        if student_channels is None or teacher_channels is None:
            raise ValueError("SKDDistillation requires student_channels and teacher_channels")
        ...
        for idx in self.layer_indices:
            self.loss_modules[str(idx)] = SKD_Loss(patch_size=patch_size, eps=eps)
        ...
        **kwargs,
        ...
        for idx in self.layer_indices:
            ...
            if student_feat is None or teacher_feat is None:
                raise ValueError(
                    f"SKDDistillation requires '{key}' features; "
    ...
    def to(self, device: torch.device) -> "SKDDistillation":
    ...
    def compute_stage_loss(
        ...
        **kwargs,
        ...
        if str(stage_idx) not in self.loss_modules:
            raise ValueError(f"Stage {stage_idx} not configured for SKDDistillation")

        module = self.loss_modules[str(stage_idx)]
        stage_loss = module(student_feat, teacher_feat)
        weight = float(self.stage_weights.get(stage_idx, 1.0))
        weighted_loss = stage_loss * weight
        return weighted_loss, {"skd": stage_loss}


__all__ = ["SKDDistillation"]
