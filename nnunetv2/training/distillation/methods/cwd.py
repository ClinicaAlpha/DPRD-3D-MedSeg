"""
Channel-wise distillation (CWD) wrapper.
"""
...
from nnunetv2.utilities.kd_losses import CWD_Loss

from .base import DistillationMethod


class CWDDistillation(DistillationMethod, nn.Module):
    """
    Channel-wise distillation for segmentation.

    Config parameters:
        student_channels (Sequence[int])
        teacher_channels (Sequence[int])
        norm_type: 'none', 'channel', 'spatial', 'channel_mean'
    ...
    def __init__(self, **config):
        ...
        if student_channels is None or teacher_channels is None:
            raise ValueError("CWDDistillation requires student_channels and teacher_channels")
        ...
        divergence = config.get("divergence", "mse")
        temperature = config.get("temperature", 1.0)

        self.loss_modules = nn.ModuleDict()
        for idx in self.layer_indices:
            self.loss_modules[str(idx)] = CWD_Loss(
        ...
        **kwargs,
        ...
        for idx in self.layer_indices:
            ...
            if student_feat is None or teacher_feat is None:
                raise ValueError(
                    f"CWDDistillation requires '{key}' features; "
    ...
    def to(self, device: torch.device) -> "CWDDistillation":
    ...
    def compute_stage_loss(
        ...
        **kwargs,
        ...
        if str(stage_idx) not in self.loss_modules:
            raise ValueError(f"Stage {stage_idx} not configured for CWDDistillation")
        ...
        return weighted_loss, {"cwd": stage_loss}


__all__ = ["CWDDistillation"]
