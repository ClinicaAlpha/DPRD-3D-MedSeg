...
try:
    ...
    cp = None
    cp_binary_erosion = None
    cp_distance_transform_edt = None
    cp_generate_binary_structure = None
    HAS_CUPY = False
...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="auto",
        help=(
            "Compute device for boundary metrics. "
            "'cuda' uses CuPy if available; 'auto' prefers CUDA when CuPy is available."
        ),
    )
...
def get_device_mode(device_arg: str) -> str:
    ...
    if device_arg == "cuda":
        if not HAS_CUPY:
            raise RuntimeError("Requested --device cuda but CuPy is not available in this environment.")
        return "cuda"
...
def main(argv: List[str] | None = None) -> None:
    ...
    # 打印output directory
    print(f"Output directory: {os.path.dirname(save_csv)}")
    print(f"Output file: {save_csv}")
    if HAS_PANDAS:
        df = pd.DataFrame(
            rows + [overall_mean_row, overall_std_row, overall_se_row],
            columns=cols)
    ...
    for cname in class_names:
        ...
        se_h_v = ses.get(f"HD95_{cname}_vox", np.nan)
        mh_m = means.get(f"HD95_{cname}_mm", np.nan)
        sh_m = stds.get(f"HD95_{cname}_mm", np.nan)
        se_h_m = ses.get(f"HD95_{cname}_mm", np.nan)
        ...
        if args.with_monai_dice:
            ...
            print(
                f"{cname:20s}  Dice(nnU-Net): {md_nu:.4f}Â±{sd_nu:.4f}   Dice(MONAI): {md_mn:.4f}Â±{sd_mn:.4f}   "
                f"HD95(vox): {mh_v:.4f}Â±{sh_v:.4f} (SE={se_h_v:.4f})   "
                f"HD95(mm): {mh_m:.4f}Â±{sh_m:.4f} (SE={se_h_m:.4f})   NSD: {mn:.4f}Â±{sn:.4f}"
            )
        else:
            print(
                f"{cname:20s}  Dice(nnU-Net): {md_nu:.4f}Â±{sd_nu:.4f}   "
                f"HD95(vox): {mh_v:.4f}Â±{sh_v:.4f} (SE={se_h_v:.4f})   "
                f"HD95(mm): {mh_m:.4f}Â±{sh_m:.4f} (SE={se_h_m:.4f})   NSD: {mn:.4f}Â±{sn:.4f}"
            )
    ...
    overall_se_h_v = ses.get("Mean_HD95_vox", np.nan)
    overall_h_m = means.get("Mean_HD95_mm", np.nan)
    overall_sh_m = stds.get("Mean_HD95_mm", np.nan)
    overall_se_h_m = ses.get("Mean_HD95_mm", np.nan)
    ...
    if args.with_monai_dice:
        ...
        print(
            f"{'OVERALL':20s}  Dice(nnU-Net): {overall_d_nu:.4f}Â±{overall_sd_nu:.4f}   Dice(MONAI): {overall_d_mn:.4f}Â±{overall_sd_mn:.4f}   "
            f"HD95(vox): {overall_h_v:.4f}Â±{overall_sh_v:.4f} (SE={overall_se_h_v:.4f})   "
            f"HD95(mm): {overall_h_m:.4f}Â±{overall_sh_m:.4f} (SE={overall_se_h_m:.4f})   NSD@{tau_print}mm: {overall_n:.4f}Â±{overall_sn:.4f}"
        )
    else:
        print(
            f"{'OVERALL':20s}  Dice(nnU-Net): {overall_d_nu:.4f}Â±{overall_sd_nu:.4f}   "
            f"HD95(vox): {overall_h_v:.4f}Â±{overall_sh_v:.4f} (SE={overall_se_h_v:.4f})   "
            f"HD95(mm): {overall_h_m:.4f}Â±{overall_sh_m:.4f} (SE={overall_se_h_m:.4f})   NSD@{tau_print}mm: {overall_n:.4f}Â±{overall_sn:.4f}"
        )
